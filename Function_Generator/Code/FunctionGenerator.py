...
#evaluate the content of a wave w at point x
def eval(w,x):
    m,s,p=1.0,0.0,0.0
...
#wave class
class waveform:
    # make a full scale 1kHz sine wave by default
    vgnd = 0.5
    dc = 0.0
    amplitude = 0.5
    freq = 1000
    ...
    phase = 0.0
    replicate = 1
    # want it to forget self
    func = staticmethod(sine) 
    # turn it on and off
    def run(self,x):
        global ibuf
        if x:
            startSM()
            setupwave(wavbuf[ibuf],self); ibuf=(ibuf+1)%2
        else:
            stopSM()
